...
Valami furcsa dolog történt, amikor megváltoztattuk a kvantor területét, mivel a PL-hez tartózó implikáció egy lényeges logikai művelet. Ahhoz hogy magmaradjon az eredeti jelentése, érdemes lenne megváltoztatnunk a kvantort: $\exists x Gx \eif Gl$ jelentse azt, mint $\forall x (Gx \eif Gl)$, és $\exists x (Gx \eif Gl)$ jelentse ugyanazt, mint $\forall x Gx \eif Gl$.
...
Ez a furcsaság nem merül fel másik összeköttetésben vagy ha a változó a feltételezésnek a következménye. Például, $\exists x Gx \eand Gl$ ha ugyanazt jelenti, mint $\exists x (Gx \eand Gl)$, és $Gl \eif \exists x Gx$ jelentése megegyezik $\exists x(Gl \eif Gx)$ jelentésével. 
...
\end{earg}
\begin{earg}
 27. Adina egy szakképzett sebész.
\end{earg}
Let the UD be people, let $Kx$ mean `$x$ is a skilled surgeon', and let $a$ mean Adina. Sentence \ref{surgeon1} is simply $Ka$.

Az UD jelentése legyen emberek, $Kx$ pedig jelentse azt, hogy '$x$ egy tapasztalt sebész', és $a$-t értelmezzük Adinának. A 27-es mondat egyszerűen csak $Ka$.
...
Érdemes lenne különbséget tennünk aközött, ha valaki jártas a sebészet világában vagy csupán egy sima sebész.Így ezek segítségével értelmezzük ezt a szimbolizáló kulcsot:
\begin{ekey}
\item[UD:] people
UD: emberek
\item[Gx:] $x$ is greedy.
...
Ezáltal le tudjuk fordítani ezt a vitát:
\begin{earg}
\label{surgeon2}
